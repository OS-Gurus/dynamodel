...
>(ddb: DynamoDB.DocumentClient, TableName: string) => ({
    /**
     * Create a set of typed maker functions.
     * This can be re-used with the same DB connection and table to allow multiple model interfaces
     * to be generated, where interacting with subsets of an item are separated within a service.
    ...
    make: <Props>() => ({
       ...
       */
      getProperty: (property: Paths<Props>) =>
        async (Key: HashKeys) => {
          const { Item } = await ddb.get({
            TableName,
            Key,
       ...
       */
      updateProperty: <Path extends Paths<Props>>(property: Path) =>
        async (Key: HashKeys, valueAtPath: AtPath<NonPartial<Props>, Path>) => {
       ...
       *   await updateUserName('a_user_id', 'a_user_name')
       */
      insertProperty: <Path extends Paths<Props>>(property: Path) =>
        async (Key: HashKeys, valueAtPath: AtPath<NonPartial<Props>, Path>) => {
          const paths = property.split('.')
          const value = nestedValue(paths, valueAtPath)
          for (const index of paths.keys()) {
